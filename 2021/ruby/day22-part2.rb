...
Cuboid = Struct.new(:xrange, :yrange, :zrange) do
  ...
  def cover?(other)
    xrange.cover?(other.xrange) &&
      yrange.cover?(other.yrange) &&
      zrange.cover?(other.zrange)
  end
...
instructions.each.with_index do |(toggle, cuboid), i|
  puts i
  new_system = []

  system.each do |existing|
    if cuboid.cover?(existing)
      next
    elsif intersect(existing, cuboid).empty?
      new_system.push(existing)
    else
      new_system.push(*subtract(existing, cuboid))
    end
  end

  new_system.push(cuboid) if toggle == "on"
  system = new_system
end

puts system.sum(&:size)
